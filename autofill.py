...
def file_is_real(filepath):
    return filepath and os.path.isfile(filepath) and os.path.getsize(filepath) > 0
...
class AutoFiller():
    def __init__(self, filename):
        ...
        self.q_front = queue.Queue()
        self.q_back = queue.Queue()
        self.q_cardback = queue.Queue()
        self.q_error = queue.Queue()

        self.cards_folder = os.path.join(get_currdir(), "cards")

        if not os.path.exists(self.cards_folder):
            os.mkdir(self.cards_folder)

        self.root = ET.parse(filename).getroot()
        self.setup_logger()

    def setup_logger(self):
        # https://stackoverflow.com/a/56689445/11751692
        self.logger = logging.getLogger('log')
        self.logger.setLevel(logging.DEBUG)

        # create file handler which logs even debug messages
        fh = logging.FileHandler('logs/log.log', mode="w")
    ...
    def fill_cards(self, bar: tqdm):
        # Load Custom Game Cards (63mm x 88mm) page
        self.logger.debug(
            "FILL: Visiting https://www.makeplayingcards.com/design/custom-blank-card.html")
        self.driver.get(
            "https://www.makeplayingcards.com/design/custom-blank-card.html")

        # Select card stock
        stock_dropdown = Select(
            self.driver.find_element_by_id("dro_paper_type"))
        stock_dropdown.select_by_visible_text(self.root[0][2].text)
        self.logger.debug(
            "FILL: Card stock {} selected".format(self.root[0][2].text))

        # Select number of cards
        qty_dropdown = Select(self.driver.find_element_by_id("dro_choosesize"))
        qty_dropdown.select_by_value(self.root[0][1].text)
        self.logger.debug(
            "FILL: Quantity {} selected".format(self.root[0][1].text))

        # Switch the finish to foil if the user ordered foil cards
        if self.root[0][3].text == "true":
            foil_dropdown = Select(
                self.driver.find_element_by_id("dro_product_effect"))
            foil_dropdown.select_by_value("EF_055")
            self.logger.debug("FILL: Foil selected")
        else:
            self.logger.debug("FILL: Foil skipped")
        ...
        # NOTE: Sometimes I get an exception at this point, not sure what causes it
        # I think the wait solves it
        self.wait()

        self.driver.switch_to.frame("sysifm_loginFrame")
        self.driver.execute_script(
            "javascript:document.getElementById('txt_card_number').value=" + str(self.root[0][0].text) + ";")

        # Select "different images" for front
        self.driver.execute_script("javascript:setMode('ImageText', 0);")
        self.driver.switch_to.default_content()

        self.logger.debug("FILL: Fronts")

        # Insert card fronts
        for i in range(len(self.cardsinfo_front)):
            curr_card = self.q_front.get()
            if curr_card != ("", ""):
                pid = self.upload_card(curr_card[0])
                self.insert_card(pid, curr_card[1])
            bar.update(1)

        self.logger.debug("FILL: Done with fronts")

        # Page through to backs
        self.driver.execute_script("javascript:oDesign.setNextStep();")
        try:
        ...
        self.logger.debug("FILL: Backs")

        self.wait()
        self.driver.switch_to.frame("sysifm_loginFrame")

        if len(self.root[2]) == 0:
            # Same cardback for every card
            self.logger.debug("FILL: 1 cardback foreach card")

            self.driver.execute_script("javascript:setMode('ImageText', 1);")
            self.driver.switch_to.default_content()

            # Pull the common cardback card info off the queue, then upload and insert it
            curr_card = self.q_cardback.get()
            if curr_card != ("", ""):
                pid = self.upload_card(curr_card[0])
                self.insert_card(pid, [0])
            self.logger.debug("FILL: Done with 1 cardback")
            bar.update(1)
        else:
            # Different cardbacks
            self.logger.debug("FILL: Different cardbacks")
            self.driver.execute_script("javascript:setMode('ImageText', 0);")
            self.driver.switch_to.default_content()

            # Insert specified cardbacks
            cards_with_backs = []
            for i in range(0, len(self.cardsinfo_back)):
                curr_card = self.q_back.get()
                if curr_card != ("", ""):
                    pid = self.upload_card(curr_card[0])
                    self.insert_card(pid, curr_card[1])
            ...
            self.logger.debug("FILL: Done with different cardbacks")

            # Determine which slots require the common cardback
            # TODO: Is there a more efficient way to do this? Look at DOM instead?
            total_cards = int(self.root[0][0].text)
            cards_needing_backs = [x for x in range(
                0, total_cards) if x not in cards_with_backs]

            # Upload and insert the common cardback
            curr_card = self.q_cardback.get()
            if curr_card != ("", ""):
                pid = self.upload_card(curr_card[0])
                self.insert_card(pid, cards_needing_backs)
            bar.update(1)
    ...
    def download_card(self, bar: tqdm, cardinfo):
        card_item = ("", "")
        try:
            ...
            try:
                ...
                try:
                    ...
                    print("cant get filename so gonna exit")
                    self.error(
                        "Failed to retrieve filename for image with ID < {} >".format(file_id))
                ...
                if not file_is_real(filepath):
                    ...
                    try:
                            ...
                            with requests.post(
                                ...
                                if len(filecontents) > 0:
                                    ...
                                    with open(filepath, "bw") as f:
                                        f.write(
                                            np.array(filecontents, dtype=np.uint8))
                        ...
                        if not image_downloaded:
                            ...
                            self.error("DL: Failed {}:\nhttps://drive.google.com/uc?id={}&export=download".format(
                                filename, file_id))
                        else:
                            self.logger.debug("DL: Done {}".format(filename))

                    except requests.exceptions.Timeout:
                        # Failed to download image because of a timeout error - add it to error queue
                        self.error("DL: Failed (timeout) {}:\nhttps://drive.google.com/uc?id={}&export=download".format(
                            filename, file_id))
                else:
                    self.logger.debug("DL: Skipping {}".format(filename))
            ...
            if file_is_real(filepath) and filename:
            ...
            self.error(
                "DL: General exception:\nhttps://drive.google.com/uc?id={}&export=download".format(file_id))
            self.logger.error(e)

        # Add to the appropriate queue
        if file_face == "front":
            self.q_front.put(card_item)
        elif file_face == "back":
            self.q_back.put(card_item)
        elif file_face == "cardback":
            self.q_cardback.put(card_item)

        # Increment progress bar
        bar.update(1)

    def upload_card(self, filepath):
        if file_is_real(filepath):
            filename = filepath.split("/")[-1]

            num_elems = len(self.driver.find_elements_by_xpath(
                "//*[contains(@id, 'upload_')]"))
            ...
            while progress_container.value_of_css_property("display") == "none":
                ...
                self.driver.find_element_by_xpath(
                    '//*[@id="uploadId"]').send_keys(filepath)
                time.sleep(1)
                progress_container = self.driver.find_element_by_id(
                    "divFileProgressContainer")
            ...
            while True:
                try:
                    ...
                    if len(elem) > num_elems:
                        ...
                        self.logger.debug("UP: Done {}".format(filename))
                    ...
                    if timeout_counter > 180:
                        ...
                        self.error(
                            "UP: Failed to upload image to MPC at path < {} >".format(filename))
                        return ""
    ...
    def insert_card(self, pid, slots):
        if pid != "":
            # Use mpc's JS functions to insert cards without simulated drag/drop
            self.driver.execute_script("javascript: l = PageLayout.prototype")
            for slot in slots:
                ...
                self.wait()
            self.logger.debug("IN: Done {}".format(pid))
    ...
    def autofill(self):
        ...
        if len(self.root[1][0]) > 2:
            # XML is 3.0-onwards, and filename can be retrieved
            self.cardsinfo_front = [
                (x[0].text, x[1].text, x[2].text, "front") for x in self.root[1]]
            self.cardsinfo_back = [
                (x[0].text, x[1].text, x[2].text, "back") for x in self.root[2]]
        else:
            # XML is pre-3.0, and filename must be retrieved from google API request
            self.cardsinfo_front = [
                (x[0].text, x[1].text, "", "front") for x in self.root[1]]
            self.cardsinfo_back = [(x[0].text, x[1].text, "", "back")
                                   for x in self.root[2]]

        cardsinfo_cardback = [(self.root[-1].text, "", "", "cardback")]
        cardsinfo = self.cardsinfo_front + self.cardsinfo_back + cardsinfo_cardback
        ...
        with ThreadPoolExecutor(max_workers=5) as pool, \
            ...
            pool.map(partial(self.download_card, dl_progress), cardsinfo)
            # Launch the main webdriver automation function
            self.fill_cards(ul_progress)
            dl_progress.close()
            ul_progress.close()
...
def find_xml():
    ...
    tree = None
    filenames = ["cards.xml", "cards.xml.txt",
                 "cards.txt.xml", "cards.xml.xml"]
...
if __name__ == "__main__":
    ...
    # filename = "decklists/8_tokens_(26).xml"
